...
vocab_file = "CNNewsClassification/wordVectorembed/vocab.pkl"  # 之前生成的存储词及其id的文件
...
def train(model, device, model_name, lr, train_dataloader, dev_dataloader, postProc=None):
    ...
    writer = SummaryWriter(log_dir='CNNewsClassification/log/%s.'%(model_name) + time.strftime('%m-%d_%H.%M', time.localtime()))  # 实例化SummaryWriter
    num_epochs = 3  # 设置训练次数
        ...
        for i, (trains, labels) in enumerate(train_dataloader):
            ...
            if total_batch % 100  == 0:
                ...
                if dev_loss < dev_best_loss:
                    ...
                    torch.save(model, 'CNNewsClassification/checkpoint/%s_model.%.2f'%(model_name, dev_loss))
                    improve = '*'  # 设置标记
...
def trainTextCNN():
    train_dataset = MyData(tokenize_fun=tokenize_textCNN, filename='CNNewsClassification/datasets/train.txt')
    dev_dataset = MyData(tokenize_fun=tokenize_textCNN, filename='CNNewsClassification/datasets/dev.txt')
